      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-400 mt-1">Record cash inflows and outflows against parties</p>
          </div>
          <Link href="/vouchers/new" className="w-full sm:w-auto justify-center bg-blue-600 text-white px-5 py-2.5 rounded-lg shadow hover:bg-blue-700 transition-colors font-medium flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
          <div className="bg-card border border-border rounded-xl p-4 sm:p-6 shadow-sm text-center">
            <p className="text-gray-400 text-xs sm:text-sm uppercase tracking-wider font-medium mb-1">Total Vouchers</p>
            <p className="text-2xl sm:text-3xl font-bold text-white">{vouchers.length}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-4 sm:p-6 shadow-sm text-center">
            <p className="text-gray-400 text-xs sm:text-sm uppercase tracking-wider font-medium mb-1">Money Paid Out</p>
            <p className="text-2xl sm:text-3xl font-bold text-red-400">₹{totalPayments.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-4 sm:p-6 shadow-sm text-center">
            <p className="text-gray-400 text-xs sm:text-sm uppercase tracking-wider font-medium mb-1">Money Received</p>
        ...
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          {loading ? (
            <div className="p-12 text-center text-gray-500 text-sm">Loading vouchers...</div>
          ) : vouchers.length === 0 ? (
                ...
                {vouchers.map((v: any) => (
                  <div key={v.id} className="p-4 space-y-2.5">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-white truncate max-w-[200px]">{v.party_name}</span>
                      <span className="text-gray-400 font-mono">{v.date}</span>
                    </div>

                    {v.narration && (
                      <p className="text-xs text-gray-400 truncate">{v.narration}</p>
                    )}

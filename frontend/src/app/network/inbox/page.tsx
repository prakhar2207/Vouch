            ...
            <>
              ...
              <div className="block md:hidden divide-y divide-border/60">
                {filteredRequests.map((req) => {
                  const taxable = req.payload.items?.reduce((acc, i) => acc + (i.taxable_amount || 0), 0) || 0;
                  return (
                    <div key={req.id} className="p-4 space-y-3 bg-card hover:bg-zinc-900/30 transition-colors">
                ...
                {selectedReq.status === "PENDING" && (
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2.5 bg-red-600/10 hover:bg-red-600/20 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold transition-colors cursor-pointer text-center"
                    >
                      Reject Invoice
                    </button>
                    <button

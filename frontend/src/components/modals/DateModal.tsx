...
export default function DateModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="w-full max-w-md bg-muted/50 border border-input rounded-2xl shadow-2xl p-6 text-foreground space-y-5">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-input text-foreground p-3 rounded-xl text-base font-semibold outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-xs">
            <button
              type="button"
              onClick={() => setPreset(0)}
              className="p-2 bg-muted hover:bg-zinc-700 text-foreground rounded-lg border border-input font-medium transition-colors"
            >
              Today
            </button>
            <button
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={() => setIsDateOpen(false)}
              className="px-4 py-2 bg-muted hover:bg-zinc-700 text-foreground/80 rounded-lg text-sm font-medium transition-colors"
            >
              Cancel (Esc)
            </button>
            <button
              type="submit"
              className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-foreground rounded-lg text-sm font-bold shadow-md transition-colors"
            >
              Apply Date (Enter)
            </button>
          </div>
